...
public class AlgoritmoClash {
	static int[] hijo;
	static int[] solucionMutada;
	static int[] solucionInicial;
	static ArrayList<IndividuoAlgoritmoClash> padresCrossOver;
	static ArrayList<IndividuoAlgoritmoClash> madresCrossOver;
	static double costoSolucionInicial;
	static ArrayList<IndividuoAlgoritmoClash> padresSeleccionados;
	...
	static ArrayList<IndividuoAlgoritmoClash> memoriaMejoresSolucionesOrdenada;
	static ArrayList<IndividuoAlgoritmoClash> poblacionActual;
	static ArrayList<IndividuoAlgoritmoClash> nuevaPoblacion;
	static ArrayList<IndividuoAlgoritmoClash> copiaPoblacionActual;
	static ArrayList<IndividuoAlgoritmoClash> copiaNuevaPoblacion;
	static ArrayList<IndividuoAlgoritmoClash> poblacionSalida;
	static double sumaTotalCostosPoblacion;
	static Random random;
	static int[] puntosCrossOver;
	static IndividuoAlgoritmoClash mejorSolucion;
	...
	//Población inicial
	public static void generarPoblacionAfortunadaInicial(int[] solucionSemillaInicial, int cantidadSwappings, int tamanoPoblacion, Swap swap, boolean incluirMemoriaPrevia) {
		random = new Random();
		solucionInicial = solucionSemillaInicial.clone();//guardo la semilla para después (restarts)
		poblacionActual = new ArrayList<>();
		int contador = 0;
		while(contador<tamanoPoblacion) {//genero la población inicial a partir de una solución inicial aleatoria como semilla
			int suerte = random.nextInt(Integer.MAX_VALUE);
			if(suerte==21) {
				...
				poblacionActual.add(new IndividuoAlgoritmoClash(solucionSemillaInicial.clone(), swap.evaluarCostoSolucion(solucionSemillaInicial), suerte));
				contador++;
		...
		if(incluirMemoriaPrevia && memoriaMejoresSolucionesOrdenada!=null) {
			poblacionActual.addAll((ArrayList<IndividuoAlgoritmoClash>) memoriaMejoresSolucionesOrdenada.clone());
		}
	}
	...
	public static void generarPoblacionInicial(int[] solucionSemillaInicial, int cantidadSwappings, int tamanoPoblacion, Swap swap, boolean incluirMemoriaPrevia) {
		...
		for(int i=0;i<tamanoPoblacion;i++) {//genero la población inicial a partir de una solución inicial aleatoria como semilla 
			solucionSemillaInicial = swap.swapping(solucionSemillaInicial, cantidadSwappings);//avanzo dos espacios para generar un nuevo vecino a partior de la última solución generada
			poblacionActual.add(new IndividuoAlgoritmoClash(solucionSemillaInicial.clone(), swap.evaluarCostoSolucion(solucionSemillaInicial)));
		}
	...
	public static ArrayList<IndividuoAlgoritmoClash> seleccionarPorRuleta(ArrayList<IndividuoAlgoritmoClash> poblacionEntrada, Swap swap, double ponderadorCantidadDePadres) {
		padresSeleccionados = new ArrayList<IndividuoAlgoritmoClash>();
		...
		for(int x=0;x<poblacionEntrada.size()*ponderadorCantidadDePadres;x++) {//cuantos padres seleccionamos
			...
			for(int i=0;i<poblacionEntrada.size()-1;i++) {
				if(ruleta[i]<lanzamiento && lanzamiento<=ruleta[i+1]) {
					padresSeleccionados.add(new IndividuoAlgoritmoClash(poblacionEntrada.get(i+1).getSolucion().clone(), swap.evaluarCostoSolucion(poblacionEntrada.get(i+1).getSolucion())));
				}
			}
		}
	...
	public static ArrayList<IndividuoAlgoritmoClash> reproducir(ArrayList<IndividuoAlgoritmoClash> poblacionEntrada, int cantidadSwappings, Swap swap, double porcentajeCorteMenor, double porcentajeCorteMayor, int tamanoPoblacion){
		nuevaPoblacion = new ArrayList<>();//inicializo la nueva población
		poblacionEntrada.sort(Comparator.comparingDouble(IndividuoAlgoritmoClash::getCosto));//Se ordenan de menor costo a mayor costo
		...
		for(int i=0;i<corteMejores;i++) {
			nuevaPoblacion.add(new IndividuoAlgoritmoClash(generarNuevoIndividuoMutacion(poblacionEntrada.get(i).getSolucion(), cantidadSwappings, swap), 0.0));
		}
		...
		for(int k=corteMejores;k<cortePeores;k++) {//separo a los papás de las mamás
			if(k%2==0) {
				padresCrossOver.add(new IndividuoAlgoritmoClash(poblacionEntrada.get(k).getSolucion(), 0.0));
			}else {
				madresCrossOver.add(new IndividuoAlgoritmoClash(poblacionEntrada.get(k).getSolucion(), 0.0));
			}
		}
		...
		if(padresCrossOver.size()>madresCrossOver.size()) {//si la cantidad de padres es distinta a la de madres, tomo la menor y no cruzo al individuo que sobra
			for(int x=0;x<madresCrossOver.size();x++) {
				nuevaPoblacion.add(new IndividuoAlgoritmoClash(generarNuevoIndividuoCrossOver(padresCrossOver.get(x).getSolucion(), madresCrossOver.get(x).getSolucion(), 2), 0.0));
				nuevaPoblacion.add(new IndividuoAlgoritmoClash(generarNuevoIndividuoCrossOver(madresCrossOver.get(x).getSolucion(), padresCrossOver.get(x).getSolucion(), 2), 0.0));
			}
		...
		for(int j=cortePeores;j<poblacionEntrada.size();j++) {
			nuevaPoblacion.add(new IndividuoAlgoritmoClash(generarNuevoIndividuoMutacion(poblacionEntrada.get(j).getSolucion(), cantidadSwappings, swap), 0.0));
		}
		
		while(nuevaPoblacion.size()<=tamanoPoblacion) {
			nuevaPoblacion.add(new IndividuoAlgoritmoClash(swap.generarSolcuionInicial(swap.getMatrizF()), 0.0));
		}
...
//		}
	...
	public static ArrayList<IndividuoAlgoritmoClash> evaluarCostosDeCadaIndividuoDeLaPoblacion(ArrayList<IndividuoAlgoritmoClash> poblacionEntrada, Swap swap){
	...
	public static double calcularCostosTotalesPoblacion(ArrayList<IndividuoAlgoritmoClash> poblacionEntrada) {
		sumaTotalCostosPoblacion = 0;
		for(int i=0; i<poblacionEntrada.size(); i++) {
	...
	public static double evaluarFitnessIndividuo(IndividuoAlgoritmoClash individuo, double sumaTotalCostosPoblacion) {
		return sumaTotalCostosPoblacion - individuo.getCosto();
	}
	
	//calculo el fitness promedio de todos los individuos de la población
	public static double evaluarFitnessPromedioPoblacion(ArrayList<IndividuoAlgoritmoClash> poblacion) {
		totalFitness = 0;
	...
	public static ArrayList<IndividuoAlgoritmoClash> reemplazarPoblacionActualFitnessPromedio(ArrayList<IndividuoAlgoritmoClash> poblacionActual, ArrayList<IndividuoAlgoritmoClash> nuevaPoblacion) {
	...
	public static ArrayList<IndividuoAlgoritmoClash> reemplazarPoblacionActualMejorCosto(ArrayList<IndividuoAlgoritmoClash> poblacionActual, ArrayList<IndividuoAlgoritmoClash> nuevaPoblacion) {
	...
	//Restart
	public static void realizarRestartPoblacion(ArrayList<IndividuoAlgoritmoClash> poblacionActual, int[] solucionInicial, double porcentajeAGuardarMejoresSolucionesEnMemoria, int tamanoPoblacion, Swap swap, boolean incluirMemoriaPrevia) {
		poblacionActual.sort(Comparator.comparingDouble(IndividuoAlgoritmoClash::getCosto));
		...
		for(int i=0;i<cantidadAGuardar;i++) {
			memoriaMejoresSolucionesOrdenada.add(new IndividuoAlgoritmoClash(poblacionActual.get(i).getSolucion().clone(), swap.evaluarCostoSolucion(poblacionActual.get(i).getSolucion())));
		}
	...
	public static double obtenerMejorCostoPoblacion(ArrayList<IndividuoAlgoritmoClash> poblacion) {
		double mejorCostoPoblacion = -1;//menos 1 si no hay elementos en la población
		poblacion.sort(Comparator.comparingDouble(IndividuoAlgoritmoClash::getCosto));//ordeno la población de menor a mayor costo
		if(poblacion.size()>0) {
	...
	public static ArrayList<Double> algoritmoClash(int[] solucionSemillaInicial, int cantidadSwappings, int tamanoPoblacion, int criterioParada, int valorDetencion, Swap swap, double porcentajeCorteMenor, double porcentajeCorteMayor, boolean incluirMemoriaPrevia, int numeroDeRestarts, double porcentajeAGuardarMejoresSolucionesEnMemoria, double ponderadorCantidadDePadres, int maximoIteracionesAdaptativo, boolean isClash) {
		...
		if(isClash) {
			generarPoblacionAfortunadaInicial(solucionSemillaInicial, cantidadSwappings, tamanoPoblacion, swap, incluirMemoriaPrevia);
		}else {
		...
		while(true) {
			...
			copiaPoblacionActual = (ArrayList<IndividuoAlgoritmoClash>) poblacionActual.clone();
			...
			copiaNuevaPoblacion = (ArrayList<IndividuoAlgoritmoClash>) nuevaPoblacion.clone();
